@if ($categories->isEmpty())
    <tr>
        <td colspan="3" class="whitespace-nowrap px-6 py-2 text-center">
            <span class="text-sm text-gray-600 dark:text-neutral-400">Data kategori kosong, silahkan tambahkan
                kategori.</span>
        </td>
    </tr>
@else
    @foreach ($categories as $index => $row)
        <tr id="category-row-{{ $row->id }}">
            ...
            <td class="whitespace-nowrap px-6 py-2 text-center">
                <div class="flex items-center justify-start">
                    <span class="text-sm text-gray-600 dark:text-neutral-400"
                        id="category-name-{{ $row->id }}">{{ $row->name }}</span>
                    <div id="category-input-{{ $row->id }}" class="hidden w-full max-w-sm">
                        <form id="form-{{ $row->id }}" action="{{ route('category.update', $row->id) }}"
                            method="post">
                            @csrf
                            @method('put')
                            <input value="{{ $row->name }}" type="text" name="name"
                                class="block w-full rounded-lg border border-gray-200 px-4 py-1 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                        </form>
                    </div>
                </div>
            </td>
            <td class="whitespace-nowrap px-6 py-2">
                <div class="flex justify-end">
                    <div id="category-edit-actions-{{ $row->id }}"
                        class="group hidden items-center divide-x divide-gray-300 rounded-lg border border-gray-300 bg-white shadow-sm transition-all dark:divide-neutral-700 dark:border-neutral-700 dark:bg-neutral-700">
                        <div class="hs-tooltip inline-block">
                            <button type="button"
                                onclick="document.getElementById('form-{{ $row->id }}').submit()"
                                class="hs-tooltip-toggle inline-flex items-center justify-center gap-x-2 rounded-s-md bg-white px-1.5 py-1.5 text-sm font-semibold text-gray-800 shadow-sm hover:bg-gray-50 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800">
                    ...
                    <div id="category-actions-{{ $row->id }}"
                        ...
                        <div class="hs-tooltip inline-block">
                            <form method="POST" action="{{ route('category.destroy', $row->id) }}"
                                ...
                                @method('delete')
                                <button type="submit"
                                    class="hs-tooltip-toggle delete inline-flex items-center justify-center gap-x-2 rounded-e-md bg-white px-2 py-1.5 text-sm font-semibold text-gray-800 shadow-sm hover:bg-gray-50 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="crimson" class="size-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
