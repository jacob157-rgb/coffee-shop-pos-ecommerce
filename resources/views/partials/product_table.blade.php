@if ($products->isEmpty())
    <tr>
        <td colspan="6" class="whitespace-nowrap px-6 py-2 text-center">
            <span class="text-sm text-gray-600 dark:text-neutral-400">Data Produk kosong, <a
                    class="text-sky-600 underline" href="{{ route('product.create') }}">silahkan tambahkan
                    produk.</a></span>
        </td>
    </tr>
@else
    @foreach ($products as $index => $row)
        <tr id="product-row-{{ $row->id }}">
            ...
            <td class="size-px whitespace-nowrap">
                <div class="px-6 py-3">
                    <span
                        class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $row->category_name }}</span>
                </div>
            </td>
            <td class="size-px whitespace-nowrap">
                <div class="px-6 py-3">
